...
export function normalizeFixture(record: TxLineFixtureRecord): Fixture {
  ...
  const kickoffTime = Number(record.StartTime)
  const started = kickoffTime < Date.now()
  // A match is live if it started in the last 3 hours and is not completed
  const threeHoursMs = 3 * 60 * 60 * 1000
  const isLive = started && (Date.now() - kickoffTime < threeHoursMs) && record.GameState !== 6
  ...
  return {
    ...
    kickoff: kickoffTime,
    phase: record.GameState === 6 ? 'C' : isLive ? 'H1' : started ? 'F' : 'NS',
    homeScore: null,
    awayScore: null,
    raw: record,
  }
