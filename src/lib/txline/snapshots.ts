...
export async function getScoresRecordsRaw(matchId: string): Promise<TxLineScoresRecord[]> {
  let records: TxLineScoresRecord[] = []
  try {
    const text = await apiGet<string>(`/scores/historical/${matchId}`)
    ...
    // Ignore and let scanBuckets handle it
  }

  if (records.length === 0) {
    records = await scanBuckets<TxLineScoresRecord>('scores', matchId)
  }
...
export async function getOddsRecordsRaw(matchId: string): Promise<TxLineOddsRecord[]> {
  ...
  const filtered = records.filter(rec => rec.SuperOddsType === '1X2_PARTICIPANT_RESULT')
  const byMessage = new Map<string, TxLineOddsRecord>()
  for (const rec of filtered) byMessage.set(rec.MessageId, rec)
